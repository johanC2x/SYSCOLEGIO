...
public class ADNEmpleado {
    ...
    private static LinkedList<Empleado> Lista2(Empleado p) throws ClassNotFoundException, SQLException {
        ...
        String sql = "select * from empleados "
                + " where nombres like concat('%',?,'%') and "
                + " apellidos like concat('%',?,'%') and "
                + " DNI like concat('%',?,'%') ";
        try (Connection cn = Conexion.Cadena();
                PreparedStatement ps = cn.prepareStatement(sql)) {
//            ps.setString(1, p.getApellidos());
    ...
    public static List<Empleado> ListaDocentes(Empleado empleado) throws ClassNotFoundException, SQLException {
        String sql = "select `IdEmpleado`, `Nombres`, `Apellidos`, `DNI`, `Sexo`, `FechaNac`, `Telefono`, `Celular`, `Direccion`, `Distrito`, `Cargo`, `Especialidad`, `Estado` from empleados \n"
                + "where concat(nombres,' ',apellidos) like concat('%',?,'%')";
    ...
    // METODOS DAO
    public static LinkedList<Empleado> obtenerEmpleado() throws ClassNotFoundException, SQLException {
        int r = 0;
        LinkedList<Empleado> Lista = new LinkedList<>();
        String sql = "select * from empleados ";
        try (Connection cn = Conexion.Cadena();

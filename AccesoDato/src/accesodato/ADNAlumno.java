...
public class ADNAlumno {
    ...
    public static LinkedList<Alumno> filtrarAlumnoNoMatriculados(Alumno alumno) throws ClassNotFoundException, SQLException {
        int r = 0;
        LinkedList<Alumno> Lista = new LinkedList<>();
        String sql = "select * from alumnos where apellidos like concat('%',?,'%') and nombres like concat('%',?,'%') and dni like concat('%',?,'%') and idalumno not in (select idalumno from matriculas)";
        try (Connection cn = Conexion.Cadena();
                PreparedStatement ps = cn.prepareStatement(sql)) {
